    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Restaurant` (`id` TEXT NOT NULL, `name` TEXT NOT NULL, `description` TEXT NOT NULL, `city` TEXT NOT NULL, `pictureId` TEXT NOT NULL, `rating` REAL NOT NULL, PRIMARY KEY (`id`))');
...
class _$RestaurantDAO extends RestaurantDAO {
  _$RestaurantDAO(
    ...
    this.changeListener,
        ...
        _restaurantInsertionAdapter = InsertionAdapter(
            ...
            changeListener),
        _restaurantDeletionAdapter = DeletionAdapter(
            database,
            'Restaurant',
            ['id'],
            (Restaurant item) => <String, Object?>{
  ...
  Stream<Restaurant?> getRestaurantById(String id) {
    return _queryAdapter.queryStream('SELECT * FROM Restaurant WHERE id = ?1',
        mapper: (Map<String, Object?> row) => Restaurant(
            id: row['id'] as String,
            name: row['name'] as String,
            description: row['description'] as String,
  ...
  Future<void> deleteRestaurant(Restaurant restaurant) async {
    await _restaurantDeletionAdapter.delete(restaurant);
  }
}
